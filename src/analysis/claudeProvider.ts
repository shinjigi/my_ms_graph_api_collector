  ...
  return new Promise((resolve, reject) => {
    ...
    proc.on("error", (err) => {
      if ((err as NodeJS.ErrnoException).code === "ENOENT") {
        reject(
          new Error(
            "claude CLI non trovato — installa Claude Code o disabilita il provider CLI",
          ),
        );
      } else {
        reject(err);
...
export class ClaudeApiProvider implements AnalyzerProvider {
  ...
  async analyze(
    ...
    log.debug(
      `Usage: ${inputTokens} in / ${outputTokens} out — stop: ${stopReason}`,
    );
...
export class OpenAiCompatibleProvider extends BatchAnalyzerProvider {
  ...
  async analyzeBatch(
    ...
    log.info(
      `[${this.name}] Invio batch a endpoint OpenAI-compat (${model})...`,
    );
