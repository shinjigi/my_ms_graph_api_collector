            ...
            entries: {
                ...
                items: {
                    ...
                    properties: {
                        ...
                        entityType:    { type: Type.STRING, enum: ["UserStory", "Task", "Bug", "recurring"] },
...
export class GeminiProvider implements AnalyzerProvider {
        ...
        try {
            ...
            if (!res.ok) {
                ...
                if (res.status === 400 || res.status === 403) {
                    log.warn(`[${this.name}] API key non valida o credito esaurito (HTTP ${res.status}): ${body.slice(0, 200)}`);
                } else {
    ...
    async analyzeBatch(
        ...
        log.info(`[${this.name}] Invio batch a Gemini (${this.modelName}) — prompt ~${promptChars} chars...`);
