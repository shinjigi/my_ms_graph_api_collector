...
export class ClaudeApiProvider implements AnalyzerProvider {
  ...
  async analyzeBatch(
    ...
    log.info(
      `[${this.name}] Invio batch a Anthropic (${model}) — prompt ~${promptChars} chars...`,
    );
    ...
    log.info(
      `[${this.name}] Completato — ${inputTokens} token in / ${outputTokens} token out — stop: ${stopReason}`,
    );
    if (stopReason === "max_tokens") {
      log.warn(
        `[${this.name}] Risposta troncata (max_tokens raggiunto) — considera un batch più piccolo`,
      );
    }
