...
export async function startZucchettiSession(headless?: boolean): Promise<ZucchettiSession> {
    ...
    log.info(`ZUCCHETTI_USERNAME=${username ? `"${username}"` : "⚠️  NON IMPOSTATO"}`);
    log.info(
        `ZUCCHETTI_PASSWORD=${password ? `"${"*".repeat(password.length)}"` : "⚠️  NON IMPOSTATO"}`,
    );

    if (!username || !password) {
        throw new Error("ZUCCHETTI_USERNAME o ZUCCHETTI_PASSWORD non configurati nel .env");
    }
    ...
    try {
        ...
        const menuText = await page.evaluate(() => document.body?.innerText?.slice(0, 500));
    ...
    async function getContextFromFrame(f: Page | Frame) {
        return (await f.evaluate(() => {
            ...
            const em =
                ...
                str(win.gCodDip || win.gCodDipUC || win.pIDEMPLOY || win.IDEMPLOY || win.idEmploy);
    ...
    try {
        ...
        log.warn(`Impossibile leggere variabili dal frame principale: ${(e as Error).message}`);
    }
