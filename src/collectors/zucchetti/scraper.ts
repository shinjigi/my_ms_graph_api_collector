...
import type { ZucchettiDay, ZucchettiJustification, ZucchettiRequest } from './index';
...
async function extractHeader(page: Page): Promise<TimesheetHeader> {
    const companyInfo  = await page.locator('[id$="_LblXCompanytbl"]').filter({ visible: true }).first().innerText();
    const employeeInfo = await page.locator('[id$="_LblXEmploytbl"]').filter({ visible: true }).first().innerText();
    const month        = await page.locator('select[id$="_TxtMese"]').filter({ visible: true }).first().inputValue();
    const year         = await page.locator('select[id$="_TxtAnno"]').filter({ visible: true }).first().inputValue();
...
async function extractRow(row: Locator, header: TimesheetHeader): Promise<ZucchettiDay | null> {
    ...
    const giustificativi = await parseActivityCell(cells[4]) as ZucchettiJustification[];
...
export async function scrapeSingleDay(page: Page, targetDate: string): Promise<ZucchettiDay | null> {
    const header = await extractHeader(page);
    const rows   = await page.locator('tr[id*="_Grid1_row"]').all();
...
export function validateDay(day: ZucchettiDay): ZucchettiDay {
    ...
    approvedReqs.forEach((r: ZucchettiRequest) => {
        ...
        if (qta === 0) {
            const match = (day.giustificativi ?? []).find((g: ZucchettiJustification) => g.text === r.text);
 ...
 */
export async function patchRawZucchettiFile(date: string, day: ZucchettiDay): Promise<void> {
