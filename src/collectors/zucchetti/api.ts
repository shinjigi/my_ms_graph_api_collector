...
export async function fetchZucchettiTimesheet(
    ...
    console.log(`  [API] Cookie header (primi 200 char): ${session.cookies.slice(0, 200)}...`);
    ...
    try {
        ...
        console.error(`  [API] ❌ Errore di rete durante fetch: ${(fetchErr as Error).message}`);
        throw fetchErr;
    }
    ...
    if (!response.ok) {
        const body = await response.text().catch(() => '(impossibile leggere body)');
        console.error(`  [API] ❌ HTTP ${response.status} – body (primi 500 char):\n${body.slice(0, 500)}`);
        throw new Error(`Zucchetti API error: ${response.status} – ${body.slice(0, 200)}`);
    }

    const responseText = await response.text();
    console.log(`  [API] Response body (primi 300 char): ${responseText.slice(0, 300)}`);

    let parsed: ZucchettiApiRawResponse;
    try {
        parsed = JSON.parse(responseText) as ZucchettiApiRawResponse;
    } catch (jsonErr) {
        console.error(`  [API] ❌ Risposta non è JSON valido. Body completo:\n${responseText.slice(0, 1000)}`);
        throw new Error(`Zucchetti: risposta non JSON – ${(jsonErr as Error).message}`);
    }

    const fields = parsed.Fields ?? [];
    const data   = parsed.Data   ?? [];
    console.log(`  [API] Fields ricevuti (${fields.length}): [${fields.join(', ')}]`);
    ...
    const expectedFields = ['DATA', 'GIORNO', 'ORARIO', 'HORD', 'HECC', 'ORAINI1', 'ORAFIN1', 'DESCRIZIONE1', 'QTA1'];
    const missingFields = expectedFields.filter(f => !fields.includes(f));
    if (missingFields.length > 0) {
        console.warn(`  [API] ⚠️  Campi ATTESI ma NON trovati nella risposta: [${missingFields.join(', ')}]`);
        console.warn(`  [API]    Campi effettivamente presenti: [${fields.join(', ')}]`);
    } else {
 ...
 */
export function mapRawToZucchettiDays(raw: ZucchettiApiRawResponse): ZucchettiDay[] {
    ...
    if (Data.length === 0) {
        console.warn(`  [MAP] ⚠️  Nessuna riga da mappare – output sarà un array vuoto.`);
        return [];
    }
    ...
    const results = Data.map((row, rowIdx) => {
        const getField = (name: string) => {
            ...
            if (idx === -1 && rowIdx === 0) {
                console.warn(`  [MAP] ⚠️  Campo "${name}" non trovato nei Fields (riga ${rowIdx})`);
            }
        ...
        if (rowIdx === 0 || rowIdx % 5 === 0) {
            console.log(`  [MAP] Riga ${rowIdx.toString().padStart(2)}: date="${date}" dow="${dayOfWeek}" orario="${orario}" hOrd="${hOrd}" hEcc="${hEcc}" timbrature="${timbrature}" giust=[${giustificativi.map(g => g.text).join('|')}]`);
        }
    ...
    console.log(`  [MAP] Giorni mappati con DATA valorizzata: ${daysWithData.length} / ${results.length}`);
    console.log(`  [MAP] Range date: ${daysWithData[0]?.date ?? 'n/a'} → ${daysWithData[daysWithData.length - 1]?.date ?? 'n/a'}`);
