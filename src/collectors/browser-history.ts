...
async function copyToTemp(srcPath: string): Promise<string> {
    const tmpPath = path.join(os.tmpdir(), `sqlite-copy-${Date.now()}-${Math.random().toString(36).slice(2)}`);
...
async function queryChromeProfile(
    ...
    try {
        ...
        return results[0].values.map(row => {
            const [id, url, title, visitTime] = row as [number, string, string | null, bigint];
...
async function queryFirefoxProfile(
    ...
    try {
        ...
        return results[0].values.map(row => {
            const [id, url, title, visitDate] = row as [number, string, string | null, bigint];
...
async function writeByMonth(
    ...
    for (const month of months) {
        ...
        const merged    = await mergeByKey<BrowserVisit>(outPath, newVisits, 'visitId');
...
export async function collectBrowserHistory(force = false): Promise<string[]> {
    ...
    if (chromeProfileDirs.length === 0 && !firefoxProfileDir) {
        console.warn('CHROME_PROFILE_DIRS e FIREFOX_PROFILE_DIR non configurati — collector browser saltato.');
        return [];
    }
    ...
    try {
        ...
        console.warn(`Browser history: impossibile caricare sql.js — ${(err as Error).message}`);
        return [];
    }
    ...
    if (chromeProfileDirs.length > 0) {
        ...
        for (let i = 0; i < chromeProfileDirs.length; i++) {
            ...
            try {
                const visits = await queryChromeProfile(SQL, profileDir, sourceName, sinceMs);
                allChromeVisits.push(...visits);
                console.log(`  [Browser] Chrome ${sourceName}: ${visits.length} visite`);
            } catch (err) {
                console.warn(`  [Browser] Chrome ${sourceName}: ${(err as Error).message}`);
            }
        }

        const chromePaths = await writeByMonth(CHROME_DIR, allChromeVisits, chromeProfileDirs, force);
    ...
    if (firefoxProfileDir) {
        try {
            const visits = await queryFirefoxProfile(SQL, firefoxProfileDir, 'firefox', sinceMs);
            console.log(`  [Browser] Firefox: ${visits.length} visite`);
            const firefoxPaths = await writeByMonth(FIREFOX_DIR, visits, [firefoxProfileDir], force);
