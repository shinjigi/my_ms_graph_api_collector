...
const SPENT       = parseFloat(process.argv[3] ?? '');
const DESCRIPTION = process.argv[4] ?? '';
const DATE        = process.argv[5] ?? undefined;  // defaults to today

if (!US_ID || isNaN(SPENT) || !DESCRIPTION) {
    console.error('Usage: npm run tp:log-time -- <usId> <hours> "<description>" [YYYY-MM-DD]');
...
async function main(): Promise<void> {
    try {
        ...
        const result = await client.logTime({ usId: US_ID, spent: SPENT, description: DESCRIPTION, date: DATE });
