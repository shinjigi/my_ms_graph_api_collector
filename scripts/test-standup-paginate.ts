...
const { createGraphClient } = require("../src/graphClient") as {
  createGraphClient: () => Promise<unknown>;
};

const chatId =
  "19:meeting_Y2U2N2IyZTQtMWM3NS00MWI3LWJkMTYtM2M3ODYyN2RmYzA2@thread.v2";
...
async function run(): Promise<void> {
  ...
  do {
    ...
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const onDay = msgs.filter((m: any) =>
      m.createdDateTime?.startsWith(target),
    );
    found += onDay.length;

    const oldest = msgs.at(-1)?.createdDateTime?.slice(0, 10);
    console.log(
      `Pagina ${page}: ${msgs.length} msg, oldest=${oldest}, trovati il ${target}: ${onDay.length}`,
    );
