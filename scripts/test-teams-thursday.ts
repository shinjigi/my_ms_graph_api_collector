...
async function run(): Promise<void> {
    ...
    console.log(`Chat di tipo 'meeting': ${allChats.filter(ch => ch.chatType === 'meeting').length}`);
    ...
    activeOnDay.forEach((ch: { chatType: string; topic: string | null; lastUpdatedDateTime?: string }) =>
        console.log(`  [${ch.chatType}] ${ch.topic ?? '(no topic)'} — ${ch.lastUpdatedDateTime}`)
    );
    ...
    standupChats.forEach(ch => console.log(`  [${ch.chatType}] ${ch.topic} — ${ch.id}`));

    // Step 3: for each chat active on the target day, try fetching messages
    const chatsToCheck = activeOnDay.length > 0 ? activeOnDay : standupChats.slice(0, 5);
    ...
    for (const chat of chatsToCheck) {
        ...
        try {
            const res = await c
                ...
                .filter(`createdDateTime ge ${targetDate}T00:00:00Z and createdDateTime le ${targetDate}T23:59:59Z`)
                .top(50)
                .get();
        ...
        try {
            ...
            console.log(`      → ${msgs.length}/${all.length} messaggi per ${targetDate}`);
            printMessages(msgs);
        } catch (err) {
            console.log(`      → Errore: ${(err as Error).message}`);
        }
...
function printMessages(msgs: Array<{ createdDateTime?: string; messageType?: string; body?: { content?: string } }>): void {
    for (const m of msgs) {
