...
async function main(): Promise<void> {
  console.log(
    `--- Dettaglio US #${US_ID}${ASSIGNEE ? ` — filtro: "${ASSIGNEE}"` : ""} ---`,
  );
  try {
    ...
    console.log(`\n${us.Name}`);
    console.log(
      `Stato: ${us.EntityState?.Name ?? "-"}  |  Owner: ${us.Owner?.FullName ?? "-"}  |  Totale ore: ${us.TimeSpent ?? 0}h`,
    );

    const entries: TpTimeEntry[] = ASSIGNEE
      ? allTimes.filter((t) =>
          t.User?.FullName?.toLowerCase().includes(ASSIGNEE.toLowerCase()),
        )
      : allTimes;
    ...
    [...totals.entries()]
      .sort(([, a], [, b]) => b - a)
      .forEach(([name, hrs]) =>
        console.log(`  ${name.padEnd(28)} ${hrs.toFixed(1)}h`),
      );
