...
export const useAnalysisStore = defineStore("analysis", () => {
    ...
    async function loadWeekHints(monday: Date): Promise<void> {
        weekHints.value = await fetchWeekProposals(monday);
    }

    function getHint(tpId: number, dayIdx: number, monday: Date): ProposalEntry | null {
        const date = shiftDate(monday, dayIdx);
        const hint =
            weekHints.value[dateToString(date)]?.entries.find((e) => e.taskId === tpId) ?? null;
    ...
    async function setEntryStatus(
        date: Date,
        tpId: number,
        status: ProposalEntry["status"],
    ): Promise<void> {
        const prop = weekHints.value[dateToString(date)];
        if (!prop) return;
    ...
    function dismissHint(tpId: number, dayIdx: number, monday: Date): void {
        const date = shiftDate(monday, dayIdx);
    ...
    function clearDayHints(dayIdx: number, monday: Date): void {
        const date = shiftDate(monday, dayIdx);
        const copy = { ...weekHints.value };
        delete copy[dateToString(date)];
    ...
    async function poll() {
        ...
        try {
            ...
            if (res.status === "done" || res.status === "error") {
                ...
                if (firstDate) {
                    const monday = getMonday(new Date(firstDate));
                ...
                if (res.status === "error") {
                    ...
                    error.value =
                        errKeys.length > 0 ? res.errors[errKeys[0]] : "Errore analisi sconosciuto";
                }
            }
        } catch (err) {
    ...
    async function loadProposal(date: string | Date) {
        try {
            const d = typeof date === "string" ? new Date(date) : date;
            const res = await fetchProposal(d);
