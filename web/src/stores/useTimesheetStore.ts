...
export const useTimesheetStore = defineStore(
    ...
    () => {
        ...
        const currentMonday = ref<Date>(getMonday());
            ...
            const tp = days.value.map(
                (_, i) =>
                    +allRowsFlattened.value
                        .reduce((acc, r) => acc + getHours(r.tpId, i), 0)
                        .toFixed(1),
            );
                ...
                for (let i = 0; i < 5; i++) {
                    ...
                    for (const row of allRowsFlattened.value) {
                        ...
                        if (hasManual) {
                            ...
                            if (serverHours > 0) continue;

                            targetHours = hint.inferredHours;
                            description = hint.comment || hint.reasoning || "";
                            isHint = true;
        ...
        async function fetchWeekData(date: Date, force = false) {
            if (!force && isEqual(currentMonday.value, date) && weekData.value) return;
            loading.value = true;
            try {
        ...
        async function submitWeekHours() {
            ...
            const failedKeys = new Set(
                result.errors.map(
                    (e: any) =>
                        `${e.tpId}_${edits.find((x) => x.tpId === e.tpId && x.date === e.date)?.dayIdx}`,
                ),
            );
            edits.forEach((e) => {
        ...
        function fillDay(dayIdx: number, targetHours: number) {
            ...
            if (targetTask) {
                ...
                setHours(targetTask.tpId, dayIdx, newVal);
            }
        }
        ...
        return {
            ...
            zucWeekTotal: computed(() => days.value.reduce((a, d) => a + (d.zucHours || 0), 0)),
            windowDeltaTotal: computed(() => {
                const tp = totalsRow.value.tp.reduce((a, b) => a + b, 0);
                const zuc = days.value.reduce((a, d) => a + (d.zucHours || 0), 0);
                return +(zuc - tp).toFixed(1);
            }),

            /** Workday totals (Mon-Fri) */
            tpWorkdayTotal: computed(() =>
                totalsRow.value.tp.slice(0, 5).reduce((a, b) => a + b, 0),
            ),
            zucWorkdayTotal: computed(() =>
                days.value.slice(0, 5).reduce((a, d) => a + (d.zucHours || 0), 0),
            ),
            workdayDeltaTotal: computed(() => {
                const tp = totalsRow.value.tp.slice(0, 5).reduce((a, b) => a + b, 0);
                const zuc = days.value.slice(0, 5).reduce((a, d) => a + (d.zucHours || 0), 0);
                return +(zuc - tp).toFixed(1);
            }),
        };
    },
