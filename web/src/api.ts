...
import { dateToString, shiftDate } from "@shared/dates";

export async function fetchWeek(date: Date): Promise<ApiWeekResponse> {
    const res = await fetch(`/api/week/${dateToString(date)}`);
    if (!res.ok) {
...
export async function fetchTpWeekHours(date: Date): Promise<ApiTpWeekResponse> {
    const res = await fetch(`/api/week/${dateToString(date)}/tp-hours`);
    if (!res.ok) {
...
export async function submitWeekHours(monday: Date, edits: SubmitEdit[]): Promise<SubmitResult> {
    const res = await fetch(`/api/week/${dateToString(monday)}/submit`, {
        method: "POST",
...
export async function fetchProposal(
    ...
    const res = await fetch(`/api/proposals/${dateToString(date)}`);
    if (res.status === 404) return null;
    if (!res.ok) throw new Error(`fetchProposal: ${res.status}`);
    return res.json();
}

export async function fetchWeekProposals(monday: Date): Promise<Record<string, DayProposal>> {
    ...
    for (let i = 0; i < 5; i++) {
        ...
        if (r.status === "fulfilled" && r.value?.proposal)
            map[dateToString(dates[i])] = r.value.proposal;
    }
    return map;
}

export async function saveProposal(date: Date, patch: Partial<DayProposal>): Promise<DayProposal> {
    const res = await fetch(`/api/proposals/${dateToString(date)}`, {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
