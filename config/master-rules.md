...
BAU ("BAU - Your Team Name") contains multiple distinct User Stories. Do not collapse all generic hours into one entry. Map each activity signal to the most specific ID below.
...
Use the generic "BAU - Your Team Name" project-level US **only** if:
...
- For BAU fallback only: `"No specific signal — BAU default"`
